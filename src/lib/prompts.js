...
function deriveFlagAnswers(options) {
    return {
        ...
        initGit: undefined,
        createGithub:
            options.github === true ? true : options.skipGithub === true ? false : undefined,
        githubVisibility: options.public ? 'public' : options.private ? 'private' : undefined,
...
export async function collectAnswers({ name, options, cwd = process.cwd() }) {
    ...
    const useCurrentDirectory = requiresPrompt(flags.useCurrentDirectory, interactiveMode)
        ? await askYesNo(
              'useCurrentDirectory',
    ...
    const parentDirectory = useCurrentDirectory
        ? cwd
        : interactiveMode
          ? await askInput('parentDirectory', 'Where should the project folder be created?', cwd)
          : cwd
    ...
    const createGithub = requiresPrompt(flags.createGithub, interactiveMode)
        ? await askYesNo('createGithub', 'Create a GitHub repository?', defaults.createGithub)
        : flags.createGithub
    ...
    const githubDescription = createGithub
        ? await askInput('githubDescription', 'Repository description (optional):', '', false)
        : ''
    ...
    const addReadme = requiresPrompt(flags.addReadme, interactiveMode)
        ? await askYesNo('addReadme', 'Add a minimal README.md?', defaults.addReadme)
        : flags.addReadme
    ...
    const addEditorconfig = requiresPrompt(flags.addEditorconfig, interactiveMode)
        ? await askYesNo('addEditorconfig', 'Add a .editorconfig?', defaults.addEditorconfig)
    ...
    const addDevcontainer = requiresPrompt(flags.addDevcontainer, interactiveMode)
        ? await askYesNo('addDevcontainer', 'Add a dev container?', defaults.addDevcontainer)
